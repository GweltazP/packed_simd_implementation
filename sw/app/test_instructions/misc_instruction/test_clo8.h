...
// Count leading ones in an 8-bit number
uint8_t count_leading_ones_8(uint8_t val) {
    uint8_t count = 0;
    for (int i = 7; i >= 0; --i) {
        if (((val >> i) & 1) == 0)
            break;
...
// Software reference model for CLO8
uint32_t clo8_sw(uint32_t rs1) {
    uint8_t a0 = (uint8_t)(rs1 & 0xFF);
    uint8_t a1 = (uint8_t)((rs1 >> 8) & 0xFF);
    uint8_t a2 = (uint8_t)((rs1 >> 16) & 0xFF);
    uint8_t a3 = (uint8_t)((rs1 >> 24) & 0xFF);

    uint8_t r0 = count_leading_ones_8(a0);
    uint8_t r1 = count_leading_ones_8(a1);
    uint8_t r2 = count_leading_ones_8(a2);
    uint8_t r3 = count_leading_ones_8(a3);
...
// Perform a single CLO8 test with random input
int run_single_clo8_test(int test_num) {
    uint32_t a = ((uint32_t)rand() << 16) | (uint32_t)rand();

    uint32_t expected = clo8_sw(a);
    uint32_t result;

    // Replace with your actual hardware instruction
    __asm__ volatile (
        "clo8 %0, %1"
        : "=r" (result)
        : "r" (a)
