...
uint32_t srli8_sw(uint32_t a, uint32_t imm) {
...
int run_single_srli8_test(int test_num) {
    uint32_t a = ((uint32_t)rand() << 16) | (uint32_t)rand();
    uint32_t imm = 3;

    volatile int result;

    uint32_t expected = srl8_sw(a, imm);
    __asm__ volatile (
        "srli8 %0, %1, 3"
        : "=r" (result)
        : "r" (a)
    ...
    //        test_num, "srl8", pass ? "PASS" : "FAIL", result_bit, expected_bit, a_bit, imm);
