...
uint32_t kslra16_sw(uint32_t a, int32_t b) {
...
int run_single_kslra16_test(int test_num) {
    ...
    uint32_t expected = kslra16_sw(a, b);
    __asm__ volatile (
        "kslra16 %0, %1, %2"
        : "=r" (result)
        : "r" (a), "r" (b)
    ...
    //        test_num, "kslra16", pass ? "PASS" : "FAIL", result_bit, expected_bit, a_bit, b);
